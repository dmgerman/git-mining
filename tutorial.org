...
| blob before      |                                          |
| blob after       |                                          |
| operation        | M modified, D deleted, R rename, A added (rename only when using git mv) |
| filename         |                                          |
...
this file was renamed more than once, but never using git mv. it was done using git add, git rm

#+BEGIN_EXAMPLE
truffle/src/main/java/org/jruby/truffle/platform/posix/TrufflePOSIXHandler.java
#+END_EXAMPLE

Her is an example of how this rename is reported by different runs of git log on the commit that did the rename
...
#+BEGIN_SRC sh
git  -C /tmp/jruby log --oneline --no-abbrev --numstat --follow truffle/src/main/java/org/jruby/truffle/platform/posix/TrufflePosixHandler.java | grep '=>'
#+END_SRC
...
using --raw (way easier to parse)

#+BEGIN_SRC sh
git  -C /tmp/jruby log --oneline --no-abbrev --raw --follow truffle/src/main/java/org/jruby/truffle/platform/posix/TrufflePosixHandler.java | egrep 'R[0-9]*'$'\t'
#+END_SRC
