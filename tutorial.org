...
warning. you have to be careful about using separators that might appear in the fields. Safer to use your own
filter (see my example above)

#+BEGIN_SRC sh
git -C /tmp/jruby log  --format='%H;%an;%at;%cn;%ct' > /tmp/rip.rip
#+END_SRC
...
create table commits(cid varchar, author varchar, authortime int, committer varchar, commitdate int);
.mode csv commits
.separator ;
.import "/tmp/rip.rip" commits
...
#+BEGIN_SRC sqlite :db /tmp/mydb.sql
select author, count(*) from commits group by author order by count(*) desc limit 5;
#+END_SRC

| author                | count(*) |
|-----------------------+----------|
| Charles Oliver Nutter |     9668 |
| Chris Seaton          |     5895 |
| Thomas E. Enebo       |     3204 |
| Benoit Daloze         |     2947 |
| Kevin Menard          |     1684 |
